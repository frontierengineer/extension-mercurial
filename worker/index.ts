import type {
  ...
  WorkerWorkspaceImplementation,
  Workspace,
} from '../../types';

// The mercurial workspace kind. Lives in the mercurial extension (remove it
...
// The workspace implementation the definition's mount() returns: begin/end plus
// the create-form probes, all shelling out to hg through the daemon-local
// context.execute (the implementation runs on the workspace's own machine, so there
// is no machine to thread).
function createMercurialProvider(context: WorkerWorkspaceContext): WorkerWorkspaceImplementation {
  return {
    ...
    async checkDirectory(probe) {
      const r = await context.execute({ command: 'hg', args: ['-R', probe.directory, 'root'], cwd: null, environment: {}, timeoutMs: null });
    ...
    async checkAuth(probe) {
      ...
      const r = await context.execute({ command: 'hg', args: ['-R', directory, 'identify', 'default'], cwd: null, environment: {}, timeoutMs: 30_000 });
...
export function register(provider: WorkerProvider): void {
  ...
  // kind, registered at the top level like every worker component. The
  // declarable metadata lives on the definition (announced to the host without
  // running workspace code); mount() returns the implementation whose
